...
# Follow me to control the turtlebot base velocity
import rospy
...
emergency_stop = False  # Changed to Boolean
...
def process_and_control_movement():
    global global_image_depth, center_x, center_y, integral_error, previous_error, emergency_stop  # Add 'emergency_stop'
    ...
    while not rospy.is_shutdown():
        with image_lock:
            twist = Twist()
            if emergency_stop:
                # If emergency stop is triggered, stop all movement
                twist.linear.x = 0
                twist.angular.z = 0
                rospy.loginfo("Emergency stop triggered! Velocity set to zero.")
            else:
...
def follow_me_callback(msg):
    ...
    if msg.data:
        emergency_stop = True  # Set emergency stop to True
    else:
        emergency_stop = False  # Clear emergency stop
